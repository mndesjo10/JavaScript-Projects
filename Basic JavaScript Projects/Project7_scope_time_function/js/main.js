...
function triggerError() {
    ...
    function errorFunction() {
        ...
        // International error 1: Trying to access undefined variable
        // This will cause a ReferenceError
        console.log("User email:", userEmail);

        // International error 2: Trying error (commented out to focus on first error)
        // let numberValue = 100;
        // numberValue();
...
// Method Assignment: Function that uses new Date().getHours() method with if statement
// This function displays a message based on the current time of day
function displayTimeBasedMessage() {
    // Get the current hour (0-23) using the Date object method
    ...
    // If statement to deternime appropriate message based on the hour
    ...
    console.log("=== Time-Base Message Function Activated ===");
    ...
    if (timeElement) {
        ...
        // Change cursor back to default since it's no longer clickable
...
// If Statment Assignment: New function with custom if statements
...
function checkWeatherRecommendation() {
    ...
    const isRaining = Math.random() < 0.5; // 30% chance of rain
    ...
    if (isRaining) {
        ...
        clothing = "Wear shorts, t-shirt, and sunscreen";
        emoji = "🏖️🏊‍♂️🕶️";
    ...
    // Additional if statement to provide special advice based on extreme conditions
    ...
    resultElement.innerHTML = `
        ...
        <em>Note: Temperature is randomly generated for demonstration purposes.</em><br>
    `;
    ...
    // Debug information
    console.log("=== Weather Recommendation Generated ===");
...
function checkAgeEligibility() {
    ...
    // Get the age value and convert to number
    const age = parseInt(ageInput.value);
    
    // Validate input
    if (isNaN(ageInput.value) || age < 0) {
    ...
    if (age < 13) {
        ...
        activitySuggestion = "You can enjoy kid-friendly attractions, playgrounds, and children's shows.";
        emoji = "🧒🎪";
        backgroundColor = "#d1ecf1"; // Light blue for children

        console.log(`Age ${age}: Child category - suggesting kid-friendly activities.`);
    } else if (age >= 13 && age < 18) {
        // Teen category
        eligibilityMessage = "You are considered a Teenager.";
        activitySuggestion = "You can participate in teen programs, sports leagues, and youth clubs.";
        emoji = "👦🏀";
        ...
        if (age >= 18 && age < 30) {
            activitySuggestion = "You can enjoy nightlife, adventure sports, and career development events.";
        } else if (age >= 30 && age < 50) {
            activitySuggestion = "You can participate in professional networking, family activities, and fitness programs.";
        } else {
            activitySuggestion = "You can enjoy cultural events, travel groups, and wellness programs.";
        }
        ...
        activitySuggestion = "You can enjoy senior centers, leisure activities, retirement communities, and volunteer opportunities.";
        emoji = "👴🎨";
    ...
    if (age >= 65) {
        activitySuggestion += " As a senior, you may be eligible for special discounts and programs.";
        console.log(`Age ${age}: Qualified for senior benefits.`);
    } else if (age < 18) {
        activitySuggestion += " Parental supervision may be required for some activities.";
        console.log(`Age ${age}: May require parental supervision.`);
    } else {
        activitySuggestion += " You have full access to all adult programs.";
        console.log(`Age ${age}: Full adult access granted.`);
    }
    ...
    ageResultElement.innerHTML = `
        ...
        Suggested Activities: ${activitySuggestion} ${emoji}<br>
    ...
    console.log("Activity Suggestion:", activitySuggestion);
}
...
console.log("Available Functions: testVariables(), triggerError(), showTime()");